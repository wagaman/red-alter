...
public class LoginController {
    ...
    public ResponseBean<UserPO> userLogin(@RequestParam String username,
        ...
        //取得User相关信息用于判断前台客户登录信息是否正确
        UserPO userPO = this.userService.findFrontUserByName(username);
        ...
        if (userPO == null) {
            ...
            //判断用户账户是否到期
            String vaildDate = DateUtil.formatDate(userPO.getValidDate(), "yyyy-MM-dd");
            String nowDate = DateUtil.formatDate(new Date(), "yyyy-MM-dd");
            if (vaildDate.compareTo(nowDate) < 0) {
                return ResponseBean.errorResponse("账户已到期，请与我司联系");
            }
        ...
        //异步保存登录日志
        UserLogThreadPool.execute(() -> userLogService.saveLoginLog(userPO, info));
